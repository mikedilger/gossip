...
impl Storage {
    ...
    fn use_kv_for_settings<'a>(&'a self, rw_txn: &mut RwTxn<'a>) -> Result<(), Error> {
        if let Some(settings) = self.read_settings2()? {
            ...
            self.write_setting_num_relays_per_person(
            ...
            self.write_setting_future_allowance_secs(
                &settings.future_allowance_secs,
                Some(rw_txn),
            )?;
            ...
            self.write_setting_image_resize_algorithm(
                &settings.image_resize_algorithm,
                Some(rw_txn),
            )?;
