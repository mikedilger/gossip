...
impl Storage {
    ...
    fn use_kv_for_settings<'a>(&'a self, rw_txn: &mut RwTxn<'a>) -> Result<(), Error> {
        if let Some(settings) = self.read_settings2()? {
            ...
            self.write_setting_automatically_fetch_metadata(&settings.automatically_fetch_metadata, Some(rw_txn))?;
            ...
            self.write_setting_relay_list_becomes_stale_hours(&settings.relay_list_becomes_stale_hours, Some(rw_txn))?;
            self.write_setting_metadata_becomes_stale_hours(&settings.metadata_becomes_stale_hours, Some(rw_txn))?;
            self.write_setting_nip05_becomes_stale_if_valid_hours(&settings.nip05_becomes_stale_if_valid_hours, Some(rw_txn))?;
            self.write_setting_nip05_becomes_stale_if_invalid_minutes(&settings.nip05_becomes_stale_if_invalid_minutes, Some(rw_txn))?;
            self.write_setting_avatar_becomes_stale_hours(&settings.avatar_becomes_stale_hours, Some(rw_txn))?;
            self.write_setting_media_becomes_stale_hours(&settings.media_becomes_stale_hours, Some(rw_txn))?;
            self.write_setting_max_websocket_message_size_kb(&settings.max_websocket_message_size_kb, Some(rw_txn))?;
            self.write_setting_max_websocket_frame_size_kb(&settings.max_websocket_frame_size_kb, Some(rw_txn))?;
            self.write_setting_websocket_accept_unmasked_frames(&settings.websocket_accept_unmasked_frames, Some(rw_txn))?;
            self.write_setting_websocket_connect_timeout_sec(&settings.websocket_connect_timeout_sec, Some(rw_txn))?;
            self.write_setting_websocket_ping_frequency_sec(&settings.websocket_ping_frequency_sec, Some(rw_txn))?;
            self.write_setting_fetcher_metadata_looptime_ms(&settings.fetcher_metadata_looptime_ms, Some(rw_txn))?;
            self.write_setting_fetcher_looptime_ms(&settings.fetcher_looptime_ms, Some(rw_txn))?;
            self.write_setting_fetcher_connect_timeout_sec(&settings.fetcher_connect_timeout_sec, Some(rw_txn))?;
            self.write_setting_fetcher_timeout_sec(&settings.fetcher_timeout_sec, Some(rw_txn))?;
            self.write_setting_fetcher_max_requests_per_host(&settings.fetcher_max_requests_per_host, Some(rw_txn))?;
            self.write_setting_fetcher_host_exclusion_on_low_error_secs(&settings.fetcher_host_exclusion_on_low_error_secs, Some(rw_txn))?;
            self.write_setting_fetcher_host_exclusion_on_med_error_secs(&settings.fetcher_host_exclusion_on_med_error_secs, Some(rw_txn))?;
            self.write_setting_fetcher_host_exclusion_on_high_error_secs(&settings.fetcher_host_exclusion_on_high_error_secs, Some(rw_txn))?;
            self.write_setting_nip11_lines_to_output_on_error(&settings.nip11_lines_to_output_on_error, Some(rw_txn))?;
