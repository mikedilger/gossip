use super::{ContentSegment, GossipUi, NoteData, Page, RepostType, shatter::Span};
use crate::feed::FeedKind;
use crate::globals::GLOBALS;
use eframe::egui::{self, Context};
use egui::{RichText, Ui};
use nostr_types::{Id, IdHex, NostrBech32, PublicKey, PublicKeyHex, Tag, NostrUrl};
                                ...
                                match note.repost {
                                    ...
                                    | Some(RepostType::Kind6Mention) => {
                                        for (i, cached_id) in note.cached_mentions.iter() {
                                            if *i == *num {
                                                if let Some(note_data) = app.notes.get(cached_id) {
                                                    // TODO block additional repost recursion
                                                    super::render_repost(app, ui, ctx, note, &note_data, content_margin_left, bottom_of_avatar);
                                                    render_link = false;
                                                }
                                            }
                                        }
...
pub(super) fn render_hyperlink<'a>(
    ui: &mut Ui,
    note: &NoteData,
    linkspan: &Span,
) {
...
pub(super) fn render_event_link(app: &mut GossipUi, ui: &mut Ui, note: &NoteData, id: &Id) {
