...
impl Storage {
    pub(super) fn m10_migrate<'a>(&'a self, prefix: &str, txn: &mut RwTxn<'a>) -> Result<(), Error> {
        ...
        // Migrate
        self.m10_populate_event_tag_index(txn)?;

        Ok(())
    }

    fn m10_populate_event_tag_index<'a>(&'a self, txn: &mut RwTxn<'a>) -> Result<(), Error> {
        let loop_txn = self.env.read_txn()?;
        for result in self.db_events1()?.iter(&loop_txn)? {
            let (_key, val) = result?;
            let event = EventV1::read_from_buffer(val)?;
            self.m10_write_event_tag_index1_event1(&event, txn)?;
        }
    ...
    pub fn m10_write_event_tag_index1_event1<'a>(
        &'a self,
        event: &EventV1,
        txn: &mut RwTxn<'a>,
    ) -> Result<(), Error> {
